...
#author -durga
#controller - pox
# l2 switching - transparent mode added
...
#   > check for the destination mac address , drop if the packet is a LLDP packet.
#   > if the packet is a multicast packet, broadcast the same.
...
class L2_switch (EventMixin):
        ...
        # processing the packet
                flowmsg = self.processPacket(dstmacaddr,inport,self.transparent)
        ...
        def processPacket(self,destmacaddr,srcport,transparent):# frame flow entries to forward the packets based on entries made in macaddrtable
                LLDPMACLST = ['01:80:c2:00:00:0e','01:80:c2:00:00:03','01:80:c2:00:00:00']
                msg = of.ofp_flow_mod() #of mod message to be sent to switch

                if destmacaddr in LLDPMACLST and not transparent: # incase the dest mac is LLDP and switch is non-transparent , drop the packet
                        msg.actions.append(of.ofp_action_output(port = of.OFPP_NONE))

                elif destmacaddr.is_multicast: # if mulicast packet, then flood
...
def launch():
        print "in launch.."
        core.registerNew(L2_switch,False) #registering the component to the core . all the attributes while intialisation can be passed after the module . False here refers that the switch is not a transparent switch
